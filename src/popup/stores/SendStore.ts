...
export default class SendStore {
  ...
  @observable public maxBitstashSend?: number = INIT_VALUES.maxBitstashSend;
  ...
  @computed public get receiverFieldError(): string | undefined {
    return isValidAddress(this.app.sessionStore.isMainNet, this.receiverAddress)
      ? undefined : 'Not a valid Bitstash address';
  }
  ...
  public init = () => {
    ...
    chrome.runtime.sendMessage({ type: MESSAGE_TYPE.GET_QRC_TOKEN_LIST }, (response: any) => {
      ...
      this.tokens.unshift(new QRCToken('Bitstash Token', 'STASH', 8, ''));
    ...
    chrome.runtime.sendMessage({
      type: MESSAGE_TYPE.GET_MAX_STASH_SEND,
    });
  }
  ...
  private handleMessage = (request: any) => {
    switch (request.type) {
      ...
      case MESSAGE_TYPE.SEND_TOKENS_FAILURE:
        ...
        break;
      case MESSAGE_TYPE.GET_MAX_STASH_SEND_RETURN:
        const bitstashToken = this.tokens[0];
        this.maxBitstashSend = request.maxBitstashAmount / (10 ** bitstashToken.decimals);
        break;
      default:
        break;
    }
