...
export function showModal(width: number, height: number, style: any = {}, src?: string): Promise<HTMLIFrameElement> {
  ...
  addClass(iframe, 'bitstash-extension-modal');

  const backdrop = document.createElement('div');
  addClass(backdrop, 'bitstash-extension-modal-backdrop');

  const html = document.documentElement;
  addClass(html, 'bitstash-extension-modal-wrap');
...
export function closeModal() {
  const iframe = document.querySelector('iframe.bitstash-extension-modal');

  if (iframe) {
    iframe.remove();
    const backdrop = document.querySelector('.bitstash-extension-modal-backdrop');

    if (backdrop) { backdrop.remove(); }

    removeClass(document.documentElement, 'bitstash-extension-modal-wrap');
  }
}
