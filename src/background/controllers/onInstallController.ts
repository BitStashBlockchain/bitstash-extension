...
export default class OnInstallController extends IController {
  constructor(main: BitstashController) {
    ...
    * dapp apps using Bitstash. We want to refresh these tabs so that they can
    ...
    * injected scripts from prior versions of Bitstash.
    * To get refreshed, dapp tabs must implement the
    * handleBitstashInstallOrUpdate event listener described in the Readme.
    */
  ...
  private refreshTab(tab: chrome.tabs.Tab) {
     // Tells the content script to post a msg to the inpage window letting it know that Bitstash was installed or updated.
     chrome.tabs.executeScript(tab.id!, {code:
      `window.postMessage(
        {
          message: { type: 'BITSTASH_INSTALLED_OR_UPDATED' }
        },
        '*'
      )`,
    });
