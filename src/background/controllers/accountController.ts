...
import { Wallet as BitstashWallet } from 'bitstashjs-wallet';
import assert from 'assert';

import BitstashController from '.';
import IController from './iController';
import { MESSAGE_TYPE, STORAGE, NETWORK_NAMES, BITSTASH_ACCOUNT_CHANGE } from '../../constants';
...
export default class AccountController extends IController {
  ...
  */
  public addAccountAndLogin = async (accountName: string, privateKeyHash: string, wallet: BitstashWallet) => {
  ...
  */
  public saveToFile = (accountName: string, mnemonic: string) => {
    ...
    element.download = `bitstash_${accountName}_${timestamp}.bak`;
    element.click();
  ...
  */
  public onAccountLoggedIn = async (isSessionRestore = false) => {
     ...
     * reopening the popup, we don't need to send the SEND_INPAGE_BITSTASH_ACCOUNT_VALUES event to
     * the inpage because window.bitstash.account has not changed.
     */
    if (!isSessionRestore) {
      this.main.inpageAccount.sendInpageAccountAllPorts(BITSTASH_ACCOUNT_CHANGE.LOGIN);
    }
  ...
  */
  private recoverFromPrivateKeyHash(privateKeyHash: string): BitstashWallet {
  ...
  private getPrivateKeyHash(wallet: BitstashWallet) {
  ...
  */
  private getWalletInfo = async (sendInpageUpdate = true) => {
    ...
    if (infoDidUpdate) {
      ...
      if (sendInpageUpdate) {
        this.main.inpageAccount.sendInpageAccountAllPorts(BITSTASH_ACCOUNT_CHANGE.BALANCE_CHANGE);
      }

      this.updateAndSendMaxBitstashAmountToPopup();
    }
  }
  ...
  * @param receiverAddress The address to send Bitstash to.
  ...
  */
  private sendTokens = async (receiverAddress: string, amount: number, transactionSpeed: TRANSACTION_SPEED) => {
    ...
    */
    try {
      ...
      const feeRate = rates[transactionSpeed]; // satoshi/byte; 500 satoshi/byte == .005 STASH/KB
      if (!feeRate) {
  ...
  /**
   * We update the maxBitstash amount under 2 scnearios
   * 1 - When wallet.info has been updated because a new balance has a new maxBitstashSend
   * 2 - Whenever the maxBitstashSend is requested, because even if the balance does not
   * change, the available UTXOs can change(which causes a change in maxBitstashSend).
   * For instance when a user sends Bitstash, but that transaction has not confirmed yet,
   * the balance will not change, but calcMaxBitstashSend is able to account for those
   * unconfirmed UTXOs and update maxBitstashSend accordingly.
   */
  private updateAndSendMaxBitstashAmountToPopup = async () => {
    if (!this.loggedInAccount || !this.loggedInAccount.wallet || !this.loggedInAccount.wallet.qjsWallet) {
      throw Error('Cannot calculate max balance with no wallet instance.');
    }

    const calcMQSPr = this.loggedInAccount.wallet.calcMaxBitstashSend(this.main.network.networkName);
    calcMQSPr.then(() => {
      chrome.runtime.sendMessage({ type: MESSAGE_TYPE.GET_MAX_STASH_SEND_RETURN,
        maxBitstashAmount: this.loggedInAccount!.wallet!.maxBitstashSend });
    });
  }
    ...
    try {
      switch (request.type) {
        ...
        case MESSAGE_TYPE.GET_WALLET_INFO:
          ...
          break;
        case MESSAGE_TYPE.GET_STASH_USD:
          sendResponse(this.loggedInAccount && this.loggedInAccount.wallet
            ? this.loggedInAccount.wallet.bitstashUSD : undefined);
          break;
        case MESSAGE_TYPE.VALIDATE_WALLET_NAME:
          sendResponse(this.isWalletNameTaken(request.name));
          break;
        case MESSAGE_TYPE.GET_MAX_STASH_SEND:
          this.updateAndSendMaxBitstashAmountToPopup();
          break;
        default:
          break;
